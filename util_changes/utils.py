...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        print("in sitepackages generate funcion ----")
        time_list = []
        ...
        print("Going into the iff conditions ----")
        ...
        print("Going into different generation modes")
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result,time_list = self.assisted_decoding(
                input_ids,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            result,time_list = self._greedy_search(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
            ...
            result,time_list = self._contrastive_search(
                input_ids,
            ...
            # 13. run beam search
            result,time_list = self._beam_search(
                input_ids,
                beam_scorer,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
            ...
            # 14. run beam sample
            result,time_list = self._beam_sample(
                input_ids,
                beam_scorer,
                logits_processor=prepared_logits_processor,
                logits_warper=logits_warper,
            ...
            # 13. run beam search
            result,time_list = self._group_beam_search(
                input_ids,
                beam_scorer,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
            ...
            # 13. run beam search
            result,time_list = self._constrained_beam_search(
                input_ids,
    ...
    def _contrastive_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            timing_list.append(time.time() - token_start_time)  #########################################
            if streamer is not None:
    ...
    def _greedy_search(
        ...
        streamer: Optional["BaseStreamer"] = None,
        **model_kwargs,
    ) -> Tuple[Union[GenerateNonBeamOutput, torch.LongTensor],list]:
        r"""
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            # print("input_ids in loop ",i," - " ,input_ids)
            timing_list.append(time.time() - token_start_time)  #########################################
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            model_kwargs = self._update_model_kwargs_for_generation(
        ...
        if return_dict_in_generate:
            print("In return_dict_in_generate")
            if self.config.is_encoder_decoder:
                print("GenerateEncoderDecoderOutput")
                return GenerateEncoderDecoderOutput(
                    sequences=input_ids,
                    scores=scores,
                    logits=raw_logits,
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        if return_dict_in_generate:
            ...
            return sequence_outputs["sequences"],timing_list
    ...
    def _constrained_beam_search(
        ...
        **model_kwargs,
        ...
        ```"""
        timing_list = []
        print("In constrained beam search")
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
